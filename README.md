...
```bash
docker run --name postgresql -d quay.io/sameersbn/postgresql:9.4-5
```
...
docker run --name postgresql -d \
  -v /opt/postgresql/data:/var/lib/postgresql quay.io/sameersbn/postgresql:9.4-5
```
...
```bash
docker run --name postgresql -d \
  -e 'DB_NAME=dbname' quay.io/sameersbn/postgresql:9.4-5
```
...
```bash
docker run --name postgresql -d [OPTIONS] quay.io/sameersbn/postgresql:9.4-5
```
