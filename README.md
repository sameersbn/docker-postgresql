...
```bash
docker pull fmartingr/postgres:9.6-4
```
...
```bash
docker run --name postgresql -itd --restart always \
  fmartingr/postgres:9.6-4 -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  fmartingr/postgres:9.6-4 -c logging_collector=on
```
